...
int leUrgencia() {
	...
	while (!feof(leUrgencia)) {
		...
		fscanf(leUrgencia, "%d\t%d\t%d-%d-%d\t%s\n", &episodio[i].epUrg, &episodio[i].sns, &episodio[i].entrada.tm_mday, &episodio[i].entrada.tm_mon, &episodio[i].entrada.tm_year, &episodio[i].estado);
		printf("%d\t%d\t%d-%d-%d\t%s\n", episodio[i].epUrg, episodio[i].sns, episodio[i].entrada.tm_mday, episodio[i].entrada.tm_mon, episodio[i].entrada.tm_year, episodio[i].estado);
...
/// <summary>
/// Função para adicionar novo episódio de urgência
/// </summary>
void novoEpUrg() {

	processoClinico episodio[MAXURGENCIA];

	FILE* urgAnt = fopen("urgencias.txt", "r");			/*Abre o ficheiro utentes.txt para adicionar registo*/

	if (urgAnt == NULL) {									/*Se o ficheiro não for encontrado, mostra mensagem de erro.*/
